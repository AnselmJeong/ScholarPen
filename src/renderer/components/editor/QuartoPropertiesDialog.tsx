...
export function QuartoPropertiesDialog({ editor, blockId, onClose }: {
  ...
  const apply = () => {
    ...
    if (isFigure) Object.assign(props, { url: block.props.url, caption, altText, width: width.trim(), height: height.trim(), alignment, figureNumber: 0 });
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
    <DialogContent className="max-h-[85vh] max-w-lg overflow-y-auto"
      ...
      <DialogHeader>
        ...
        <DialogDescription>Set the reference identifier and how this block appears in Quarto output.</DialogDescription>
      </DialogHeader>
      <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); apply(); }}>
        ...
        <DialogFooter><Button type="button" variant="outline" onClick={onClose}>Cancel</Button><Button type="submit">Apply</Button></DialogFooter>
      </form>
    </DialogContent>
  </Dialog>;
}
